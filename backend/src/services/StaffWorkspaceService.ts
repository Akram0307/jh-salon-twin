...
export class StaffWorkspaceService {
  static async getAvailability(staffId: string): Promise<never> {
  ...
  static async updateAvailability(params: { staffId: string; dayOfWeek: number; startTime: string; endTime: string; isAvailable: boolean }): Promise<never> {
  ...
  static async getTimeoff(staffId: string): Promise<never> {
    log.warn({ staffId }, 'getTimeoff not yet implemented');
    throw new Error('StaffWorkspaceService.getTimeoff not implemented');
  }

  static async createTimeoff(params: { staffId: string; startDate: string; endDate: string; reason: string }): Promise<never> {
