  ...
  initialize(server: Server) {
    ...
    this.sessionCleanupInterval = setInterval(async () => {
      try {
        ...
        if (cleaned > 0) {
          log.info(`🧹 Cleaned up ${cleaned} expired PWA sessions`);
        }
      } catch (error) {
  ...
  private async handleConnection(ws: WebSocket, req: IncomingMessage) {
    ...
    if (isPWA && token) {
      try {
        ...
        log.warn({ data: error }, '⚠️ PWA client authentication failed:');
    ...
    log.info(`🔌 WebSocket client connected: ${clientId} (salon: ${salonId}, PWA: ${isPWA}, Auth: ${isAuthenticated})`);
    ...
    ws.on('message', (data: Buffer) => {
      try {
        ...
        log.error({ err: err }, 'Invalid WebSocket message:');
      }
    });
    ...
    ws.on('error', (err) => {
      log.error({ err: err }, `WebSocket error for client ${clientId}:`);
      this.clients.delete(clientId);
    });
  }
